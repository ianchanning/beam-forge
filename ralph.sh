...
# Locate the Persona: Custom file if provided, else fallback to defaults
if [[ -f "${SYSTEM_PROMPT_FILE:-}" ]]; then
    PERSONA="${SYSTEM_PROMPT_FILE}"
elif [ -f "$PERSONA_LOCAL" ]; then
    PERSONA="$PERSONA_LOCAL"
elif [ -f "$PERSONA_MOTHERSHIP" ]; then
    PERSONA="$PERSONA_MOTHERSHIP"
else
    echo "Error: Persona file not found. Set SYSTEM_PROMPT_FILE or ensure personas/killer.md exists."
    exit 1
fi
